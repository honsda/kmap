...
const KANJIVG_RADICAL_URL = 'https://raw.githubusercontent.com/yagays/kanjivg-radical/master/data/element2kanji.json';
...
const JLPT_KANJI_DATA_URL = 'https://raw.githubusercontent.com/davidluzgouveia/kanji-data/master/kanji.json';
...
async function main() {
  try {
    ...
    const supplementPath = path.join(SCRIPTS_DIR, 'cache', 'kanji-data-supplement.json');

    const shouldDownload = (filePath) => {
      if (!fs.existsSync(filePath)) return true;
      return fs.statSync(filePath).size === 0;
    };

    // 1. Download datasets if they don't exist locally or are empty
    if (shouldDownload(unicodeMapPath)) {
      await downloadFile(UNICODE_MAP_URL, unicodeMapPath);
    }
    if (shouldDownload(kanjivgRadicalPath)) {
      await downloadFile(KANJIVG_RADICAL_URL, kanjivgRadicalPath);
    }
    if (shouldDownload(kanjiAlivePath)) {
      await downloadFile(KANJI_ALIVE_RADICALS_URL, kanjiAlivePath);
    }
    if (shouldDownload(kanjidicPath)) {
      await downloadFile(KANJIDIC2_URL, kanjidicPath);
    }
    if (shouldDownload(supplementPath)) {
      await downloadFile(JLPT_KANJI_DATA_URL, supplementPath);
    }
    ...
    console.log('Loading JLPT supplement database...');
    const supplementMap = JSON.parse(fs.readFileSync(supplementPath, 'utf8'));
    ...
    for (let i = 1; i < characters.length; i++) {
      ...
      const supplement = supplementMap[literal] || {};
      ...
      // Readings
      const readingRegex = /<reading\s+r_type="([^"]+)">([^<]+)<\/reading>/g;
      let readingMatch;
      ...
      // Supplement data if missing in KANJIDIC2
      if (onyomi.length === 0 && supplement.readings_on) {
        onyomi = supplement.readings_on;
      }
      if (kunyomi.length === 0 && supplement.readings_kun) {
        kunyomi = supplement.readings_kun;
      }
      let meaningStr = meanings.join(', ');
      if (!meaningStr && supplement.meanings) {
        meaningStr = supplement.meanings.join(', ');
      }

      const freqMatch = block.match(/<freq>(\d+)<\/freq>/);
      const freq = freqMatch ? parseInt(freqMatch[1], 10) : 9999;

      const strokeMatch = block.match(/<stroke_count>(\d+)<\/stroke_count>/);
      const strokeCount = strokeMatch ? parseInt(strokeMatch[1], 10) : 99;

      const jlptMatch = block.match(/<jlpt>(\d+)<\/jlpt>/);
      const oldJlpt = jlptMatch ? parseInt(jlptMatch[1], 10) : null;
      let jlpt = oldJlpt;
      if (supplement.jlpt_new !== undefined && supplement.jlpt_new !== null) {
        jlpt = supplement.jlpt_new;
      } else if (supplement.jlpt_old !== undefined && supplement.jlpt_old !== null) {
        jlpt = supplement.jlpt_old;
      }
    ...
    // Populate all constituent elements (radicals) for every kanji in kanjiMap
    for (const [element, kanjis] of Object.entries(element2kanji)) {
      const normalizedElement = normalizationMap[element] || element;
      for (const kanji of kanjis) {
        if (kanjiMap[kanji]) {
          if (!kanjiMap[kanji].radicals) {
            kanjiMap[kanji].radicals = [];
          }
          if (!kanjiMap[kanji].radicals.includes(normalizedElement)) {
            kanjiMap[kanji].radicals.push(normalizedElement);
          }
        }
      }
    }
    ...
    for (let i = 1; i < csvRows.length; i++) {
      ...
      // Sort commonKanji by frequency rank (descending commonness) and stroke count
      commonKanji.sort((a, b) => {
        const freqA = kanjiMap[a]?.freq ?? 10000;
        const freqB = kanjiMap[b]?.freq ?? 10000;
        if (freqA !== freqB) {
          return freqA - freqB;
        }
        const strokeA = kanjiMap[a]?.strokeCount ?? 99;
        const strokeB = kanjiMap[b]?.strokeCount ?? 99;
        if (strokeA !== strokeB) {
    ...
    const kanjiJSON = JSON.stringify(kanjiMap, null, 2);
    ...
    fs.writeFileSync(path.join(PUBLIC_DATA_DIR, 'kanji_data.json'), kanjiJSON);
    ...
    fs.writeFileSync(path.join(SRC_DATA_DIR, 'kanji_data.json'), kanjiJSON);
